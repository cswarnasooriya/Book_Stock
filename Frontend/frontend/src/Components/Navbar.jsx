...
const Navbar = () => {
  ...
  return (
    <>
        <div className={`max-w-screen-2xl container mx-auto md:px-20 px-4 dark:bg-slate-900 dark:text-white fixed left-0 right-0 top-0 z-50 ${sticky ? "sticky-navbar shadow-md bg-base-200  dark:bg-slate-900 dark:text-white  duration-300 transition-all ease-in-out":""}`}>
            ...
            <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 font-semibold dark:bg-slate-900 dark:text-white ">
                {NavItem}
                
            </ul>
            </div>
            <a className="btn btn-ghost text-xl">Book Stock</a>
        </div>
        <div className="navbar-end space-x-3">
            <div className="navbar-center hidden lg:flex">
             ...
             <div className="hidden md:block">
                <label className="px-3 py-1 rounded-md border flex items-center gap-2">
                    <input type="text" className={`grow outline-none dark:bg-slate-900 dark:text-white ${sticky?"bg-base-200 font-semibold text-black":""}`} placeholder="Search" />
...

            {
                ...
                <div className="">
                    <a className="btn bg-black text-white px-8 rounded-md hover:bg-slate-800 cursor-pointer duration-300" onClick={()=>document.getElementById('my_modal_3').showModal()}>
                    Login
                    </a>
                    <Login/>
